...
// CustomerService - Customer and Address service
// 
//
public class CustomerService : ICustomerService
{
    ...
    // createAddress - Used to create a new address from AddressDTO and Customer ID
    //                 Returns 0 on business validation error.
    //                 Throws FaultException on failure from EF.
    public int createAddress(AddressDTO newAddress, int customerID)
    {
        // Validate - customer must exist for an address to be created
        if (db.Customers.Where(c => c.CustomerID == customerID).Count() < 1)
        {
            return 0;
        }
        ...
        return db.Addresses.Where(a => a == address).LastOrDefault().AddressID;
    }
